...
// syntax 1
template <typename T>
requires std::is_integral_v<T> // requires is keyword, std::integral is type trait(template class) to check if a given type is integral. or w can use "std::is_integral_v".
T add(T a) {
    return a;
}
